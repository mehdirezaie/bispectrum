...
from src.models import PowerSpectrum
...
def run_emcee(comm):
    ...
    np.random.seed(85)
    res = minimize(nlogpost, [1.09, 1.02, 0.02, 0.03, 0.04, 0.01, 0.], method="Nelder-Mead")
    assert res.success
    
    start = (res.x + res.x*0.01*np.random.randn(kw['nwalkers'], kw['ndim']))
...
if __name__ == '__main__':
    ...
    if rank == 0:
        ...
        ap.add_argument('--stat', default='pk')
        ...
        ap.add_argument('--ndim', type=int, default=7)
